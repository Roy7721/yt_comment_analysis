...
def load_data(interim_dir: str):
    ...
    try:
        ...
        for name, df in [("train", train), ("test", test)]:
            ...
            df.drop(
                df[df["clean_comment"].astype(str).str.strip() == ""].index,
                inplace=True,
            )
            if before != len(df):
...
def build_features(train, test, ngram_range, max_features, use_spacy):
    ...
    try:
        ...
        logger.debug(
            "TF-IDF done. vocab=%d, train=%s", len(tfidf.vocabulary_), X_train.shape
        )
        ...
        if use_spacy:
            ...
            train_feats = pd.DataFrame(
                [extract_custom_features(t, nlp) for t in X_train_text]
            )
            test_feats = pd.DataFrame(
                [extract_custom_features(t, nlp) for t in X_test_text]
            )
...
def save_vectorizer(tfidf, custom_columns, models_dir):
    ...
    try:
        ...
        joblib.dump(
            {"tfidf": tfidf, "custom_columns": custom_columns},
